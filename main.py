...
def checkInputExcel():
    input_folder_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "입력폴더(Excel)")
...
def generate_word_doc(corp_info, directors, shareholders, ceo_info, basic_info, shareholder_count, filepath):
    ...
    template_path = os.path.join(base_path, 'doc_templates', '발기인회의사록_template.docx')
...
def generate_word_doc_1(corp_info, directors, shareholders, ceo_info, basic_info, shareholder_count, filepath):
    ...
    template_path = os.path.join(base_path, 'doc_templates', '정관_template.docx')
